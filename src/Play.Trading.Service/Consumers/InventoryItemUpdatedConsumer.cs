...
public class InventoryItemUpdatedConsumer : IConsumer<InventoryItemUpdated>
{
    ...
    public async Task Consume(ConsumeContext<InventoryItemUpdated> context)
    {
        ...
        var inventoryItem = await _repository.GetOneAsync(item => item.UserId == message.UserId && item.CatalogItemId == message.CatalogItemId);
        if (inventoryItem == null)
        {
            inventoryItem = new InventoryItem { CatalogItemId = message.CatalogItemId, UserId = message.UserId, Quantity = message.NewTotalQuantity };
